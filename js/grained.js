...
(function(window, doc) {
      ...
      console.log(ele);
        ...
        if (options.animate) {
            while (pre--) {
                ...
                rule += prefixes[pre] + 'animation-delay: 0s;';
    ...
    //no
    //setTimeout(window.grained = grained, 3000);
     window.grained = setTimeout(grained, 10000);


     //need function for tv out effect
     function tvout() {
       console.log("tvout");
       //trigger animation
       $('.container-fluid').addClass('animate_content');
       setTimeout(function() {
          $('.container-fluid').hide();
          console.log("hide container");
          }, 1500);
        //yes, the above will tv out but also restart it
        //test final image or div

     }
     //this will handle the timing correctly
     window.tvout = setTimeout(tvout, 20000);
